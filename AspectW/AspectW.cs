...
namespace NCCUCS.AspectW
{
    public class AspectW
    {
        ...
        public void Do(Action work)
        {
            ...
            {
                // will invoke Action<Action> layer by layer
                this.Chain(work);
            }
        }
    ...
    public static partial class AspectWExtensions
    {
        ...
        [DebuggerStepThrough]
        public static AspectW Trace(this AspectW aspect, Action traceBefore, Action traceAfter, Action<Exception> traceException)
        {
            return aspect.Combine((work) =>
            {
                if (traceBefore != null)
        ...
        [DebuggerStepThrough]
        public static AspectW TraceHowLong(this AspectW aspect, Action traceBefore, Action<TimeSpan> traceAfter, Action<TimeSpan, Exception> traceException)
        {
            return aspect.Combine((work) =>
            {
                if (traceBefore != null)
        ...
        [DebuggerStepThrough]
        public static AspectW Trace(this AspectW aspect, Action<DateTime> traceBefore, Action<DateTime, TimeSpan> traceAfter, Action<DateTime, TimeSpan, Exception> traceException)
        {
            return aspect.Combine((work) =>
            {
                DateTime start = DateTime.Now;
                if (traceBefore != null)
                    traceBefore(start); // start time
                ...
                if (traceAfter != null)
                    traceAfter(finish, finish - start); // finish time, and how-long
            });
        }

        [DebuggerStepThrough]
        public static AspectW Trace(this AspectW aspect, Action traceBefore, Action traceAfter)
        {
            return aspect.Trace(traceBefore, traceAfter, null);
        }

        [DebuggerStepThrough]
        public static AspectW TraceHowLong(this AspectW aspect, Action traceBefore, Action<TimeSpan> traceAfter)
        {
            return aspect.TraceHowLong(traceBefore, traceAfter, null);
        }

        [DebuggerStepThrough]
        public static AspectW TraceHowLong(this AspectW aspect, Action<TimeSpan> traceAfter)
        {
            return aspect.TraceHowLong(null, traceAfter, null);
        }
        ...
        [DebuggerStepThrough]
        public static AspectW TraceBefore(this AspectW aspect, Action<string, object[]> traceBeforeDelegate, string message, params object[] args)
        {
            return aspect.Combine((work) =>
            {
                traceBeforeDelegate(message, args);
                work();
            });
        }

        [DebuggerStepThrough]
        public static AspectW TraceAfter(this AspectW aspect, Action traceAfter)
        {
            return aspect.Combine((work) =>
            {
                work();
        ...
        [DebuggerStepThrough]
        public static AspectW TraceAfter(this AspectW aspect, Action<string, object[]> traceAfterDelegate, string message, params object[] args)
        {
            return aspect.Combine((work) =>
            {
                work();
                traceAfterDelegate(message, args);
            });
        }

        [DebuggerStepThrough]
        public static AspectW TraceAfter(this AspectW aspect, Action<string, TimeSpan, object[]> traceAfterDelegate, string message, params object[] args)
        {
            return aspect.Combine((work) =>
            {
                DateTime start = DateTime.Now;
                ...
                traceAfterDelegate(message, howLong, args);
            });
        }
            ...
            return aspect.Combine((work) =>
            {
                for (; ; )
                {
                    ...
                    catch(TException)
                    {
                        if (retryCount-- > 0) // decide to retry or not.
                        {
                            Thread.Sleep(retryDuration); // waiting short time and then re-exec.
                            continue; // retry
                        }
                        throw; // fail & leave
                    }
        ...
        [DebuggerStepThrough]
        public static AspectW Retry(this AspectW aspect, int retryDuration, int retryCount
            , Action<Exception> recoveryAction)
        {
            return Retry<Exception>(aspect, retryDuration, retryCount, recoveryAction, (Action<Exception>)null);
        }
            ...
            return aspect.Combine((work) =>
            {
                for (; ; )
                {
                    ...
                    catch (TException ex)
                    {
                        ...
                        {
                            Thread.Sleep(retryDuration); // waiting short time and then re-execute.
        ...
        public static AspectW RetryX<TException>(this AspectW aspect, int retryDuration, int retryCount
            , Action<TException> recoveryAction
            , Func<Exception, HandleFailMethod> failHandler)
            where TException : Exception
        {
            return aspect.Combine((work) =>
            {
                for (; ; )
                {
                    ...
                    catch(TException ex)
                    {
                        ...
                        else if (failHandler != null) // had retried, but still fail!
                        {
                            ...
                            {
                                case HandleFailMethod.Ignore: // just do nothing.
                                    break;
                                case HandleFailMethod.Retry:
        ...
        [DebuggerStepThrough]
        public static AspectW HandleFail(this AspectW aspect, Func<Exception, HandleFailMethod> failHandler)
        {
            return HandleFail<Exception>(aspect, failHandler);
        }

        [DebuggerStepThrough]
        public static AspectW HandleFail<TException>(this AspectW aspect, Func<TException, HandleFailMethod> failHandler)
            where TException : Exception
        {
            return aspect.Combine((work) =>
            {
                for (; ; )
                {
                    ...
                    catch (TException ex)
                    {
                        switch (failHandler(ex))
                        {
                            ...
                            case HandleFailMethod.Retry:
                                continue; // jump to for;
                            //case HandleFailMethod.ThrowOut:
                            default:
                                throw; // throw exception out.
                        }
                    }
    ...
    public sealed class RestoreWhenFailSet
    {
        private RestoreWhenFail[] res = null;

        public RestoreWhenFailSet(params RestoreWhenFail[] restoreArg)
        {
            res = new RestoreWhenFail[restoreArg.Length];
            restoreArg.CopyTo(res, 0);
        }

        public void Restore()
        {
            if (res == null) return;
            foreach (RestoreWhenFail r in res)
                r.Restore();
        }
    }
